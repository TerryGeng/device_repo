...
inputConv = {
    alazar_api.INPUT_RANGE_PM_20_MV: 0.02,
    alazar_api.INPUT_RANGE_PM_40_MV: 0.04,
    alazar_api.INPUT_RANGE_PM_50_MV: 0.05,
    alazar_api.INPUT_RANGE_PM_80_MV: 0.08,
    alazar_api.INPUT_RANGE_PM_100_MV: 0.1,
    alazar_api.INPUT_RANGE_PM_200_MV: 0.2,
    alazar_api.INPUT_RANGE_PM_400_MV: 0.4,
    alazar_api.INPUT_RANGE_PM_500_MV: 0.5,
    alazar_api.INPUT_RANGE_PM_800_MV: 0.8,
    alazar_api.INPUT_RANGE_PM_1_V: 1.0,
    alazar_api.INPUT_RANGE_PM_2_V: 2.0,
    alazar_api.INPUT_RANGE_PM_4_V: 4.0,
    alazar_api.INPUT_RANGE_PM_5_V: 5.0,
    alazar_api.INPUT_RANGE_PM_8_V: 8.0,
    alazar_api.INPUT_RANGE_PM_10_V: 10.0,
    alazar_api.INPUT_RANGE_PM_20_V: 20.0,
    alazar_api.INPUT_RANGE_PM_40_V: 40.0,
    alazar_api.INPUT_RANGE_PM_16_V: 16.0,
    # API.INPUT_RANGE_HIFI	=	0x00000020
    alazar_api.INPUT_RANGE_PM_1_V_25: 1.25,
    alazar_api.INPUT_RANGE_PM_2_V_5: 2.5,
    alazar_api.INPUT_RANGE_PM_125_MV: 0.125,
    alazar_api.INPUT_RANGE_PM_250_MV: 0.25
}


def getInputRange(maxInput, model, impedance='50 Ohm', returnNum=False):
    if model in [alazar_api.ATS9325, alazar_api.ATS9350, alazar_api.ATS9850, alazar_api.ATS9870]:
        available = [
            alazar_api.INPUT_RANGE_PM_40_MV, alazar_api.INPUT_RANGE_PM_100_MV, alazar_api.INPUT_RANGE_PM_200_MV,
            alazar_api.INPUT_RANGE_PM_400_MV, alazar_api.INPUT_RANGE_PM_1_V, alazar_api.INPUT_RANGE_PM_2_V,
            alazar_api.INPUT_RANGE_PM_4_V
        ]
    elif model in [alazar_api.ATS9351, alazar_api.ATS9360]:
        available = [alazar_api.INPUT_RANGE_PM_400_MV]
...
class AlazarTechDigitizer():
    def __init__(self, systemId=1, boardId=1):
        ...
        handle = alazar_api.AlazarGetBoardBySystemID(systemId, boardId)
        if handle is None:
            raise Exception(
    ...
    def callFunc(self, funcName, *args, **kw):
        ...
        func = getattr(alazar_api, funcName)
        # call function
        status = func(*args)
        if status != alazar_api.ApiSuccess:
    ...
    def checkErrors(self, ignores=[]):
        ...
        if len(errors) != 0:
            ...
            raise Exception('%d, %s' % errors[-1])

    def getError(self, status):
        """Convert the error in status to a string
        """
        errorText = alazar_api.AlazarErrorToText(status)
        return str(errorText)

    def getChannelInfo(self):
        memorySizeInSamples, bitsPerSample = alazar_api.U32(), alazar_api.U8()
    ...
    def getBoardKind(self):
        ret = alazar_api.AlazarGetBoardKind(self.handle)
        return ret

    def setLEDOn(self, on=True):
        if on:
            self.callFunc('AlazarSetLED', self.handle, alazar_api.LED_ON)
        else:
            self.callFunc('AlazarSetLED', self.handle, alazar_api.LED_OFF)

    def setCaptureClock(self, SourceId, SampleRateId, EdgeId=0, Decimation=0):
        """Configure sample clock source, edge, and decimation.
        """
        if SourceId == alazar_api.EXTERNAL_CLOCK_10MHz_REF:
    ...
    def isBusy(self):
        return bool(alazar_api.AlazarBusy(self.handle))
...
class AutoDMA:
    ...
    def before(self):
        flags = alazar_api.ADMA_NPT | alazar_api.ADMA_EXTERNAL_STARTCAPTURE | alazar_api.ADMA_INTERLEAVE_SAMPLES
        if self.buffers is None:
            flags |= alazar_api.ADMA_ALLOC_BUFFERS

        recordsPerAcquisition = 0x7FFFFFFF if self.repeats<=0 else 2*self.repeats

        self.dig.beforeAsyncRead(alazar_api.CHANNEL_A | alazar_api.CHANNEL_B, 0,
    ...
    def read(self):
        ...
        scaleA = self.dig.inputRange[alazar_api.CHANNEL_A] / codeRange
        scaleB = self.dig.inputRange[alazar_api.CHANNEL_B] / codeRange
